...
const ProviderDAO = {
    ...
    createProvider: (newProvider) => {
        return new Promise((resolve, reject) => {
            ...
            // Check for duplicate provider with the same name
            db.query('SELECT * FROM provider WHERE prv_name = ?', [name])
                .then(({ results }) => {
                    if (results.length > 0) {
                        // If a provider with the same name already exists, reject with a duplicate error
                        reject({ status: 409, error: "A provider with this name already exists" });
                    } else {
